...
if(print_base) {
    ...
    translate([power_port_x+15,0,s]) cube([8,s,5]);
    ...
    if(print_cam) translate(cam_pos) translate([-cam_w/2,-cam_w/2-2,cam_mount_h+s])cam();
...
if(print_rear) translate([l+w,0,s])rotate([0,180,0])
    difference(){
        union(){
